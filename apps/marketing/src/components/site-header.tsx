...
export function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-line/80 bg-paper-raised/85 backdrop-blur-md">
      <a
        href="#main"
        className="sr-only rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground focus:not-sr-only focus:absolute focus:start-4 focus:top-3 focus:z-50"
      >
        {nav.skipToContent}
      </a>
      ...
      >
        ...
        <div className="mt-4 grid grid-cols-2 gap-2 border-t border-line pt-4">
          <a
            href={LOGIN_URL}
            className="inline-flex h-11 items-center justify-center rounded-lg border border-line text-sm font-medium text-ink"
          >
            {nav.signIn}
          </a>
