 ...
 * theme tokens via `currentColor` / CSS variables so they flip with dark mode.
 */
...
export function LineAreaChart({ points, height = 160, className, format = String, color = "var(--color-primary)" }: LineAreaChartProps) {
  const id = useId();
  const w = 600;
  const h = height;
  const padX = 8;
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none" className={cn("w-full", className)} style={{ height }}>
      ...
      {[0.25, 0.5, 0.75].map((f) => (
        <line key={f} x1={padX} x2={w - padX} y1={padY + (h - padY * 2) * f} y2={padY + (h - padY * 2) * f} stroke="var(--color-line)" strokeDasharray="3 4" strokeWidth="1" />
      ))}
...
/** Horizontal ranked bars — top products, sources, governorates. */
      ...
      {rows.map((r, i) => (
        <li key={i}>
          <div className="flex items-center justify-between gap-3 text-sm">
            ...
            <span className="shrink-0 tabular-nums text-ink-soft">{r.caption ?? format(r.value)}</span>
          </div>
...
export function FunnelBars({ stages, className }: FunnelBarsProps) {
  ...
  return (
    <ol className={cn("space-y-2", className)}>
      {stages.map((s, i) => {
        ...
        const stepRate = prev ? Math.round((s.value / Math.max(prev, 1)) * 100) : null;
        return (
          <li key={s.label} className="flex items-center gap-3 text-sm">
            <span className="w-32 shrink-0 truncate text-ink-soft">{s.label}</span>
            <div className="h-6 flex-1 overflow-hidden rounded bg-line/50">
              <div className="flex h-full items-center rounded bg-primary/85 px-2 text-xs font-medium text-white" style={{ width: `${Math.max(pct, 4)}%` }}>
                {s.value.toLocaleString()}
              </div>
            </div>
            <span className="w-12 shrink-0 text-right text-xs tabular-nums text-ink-soft">{stepRate === null ? "" : `${stepRate}%`}</span>
          </li>
        );
      })}
    </ol>
...
export function Sparkline({ values, className, color = "var(--color-primary)" }: { values: number[]; className?: string; color?: string }) {
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none" className={cn("h-7 w-24", className)}>
