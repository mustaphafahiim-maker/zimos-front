...
describe("InboxPage", () => {
  it("sends the merchant to Settings → Integrations when WhatsApp isn't connected", async () => {
    ...
    expect(currentPath()).toBe("/settings?tab=integrations");
  ...
  it("lists conversations and opens a thread with the closed-window notice", async () => {
    ...
    expect(api.listWhatsappConversations).toHaveBeenCalledWith("ws_1", expect.objectContaining({ status: "open" }));
    ...
    expect(screen.getByRole("link", { name: "Customer profile" })).toHaveAttribute("href", "/customers/cus_1");
  });
});
