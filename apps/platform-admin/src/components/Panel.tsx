...
export function Panel({
  ...
  return (
    <section
      className={cn(
        "overflow-hidden rounded-[var(--radius-card)] border border-line bg-paper-raised",
        className
      )}
    >
      {(title || actions) && (
...
export function SourceNotice({ result }: { result: Pick<WorkspaceListResult, "source" | "apiError"> }) {
  ...
  // Our Alert has no `warning` variant; the amber tokens carry the tone.
  return (
    <Alert className="mb-4 border-accent/30 bg-accent-soft text-accent-dark">
