      ...
      >
        <div className="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
          {(["operational", "degraded", "down"] as const).map((h) => (
            <div key={h} className="flex items-center justify-between rounded-[var(--radius-card)] border border-line bg-paper-raised px-4 py-3">
              <Status value={h} />
        ...
        <Alert variant="info" className="mb-4 flex items-center gap-2 border-primary/20 bg-primary-soft text-ink">
              ...
              {rows.map((p) => {
                ...
                return (
                  <TableRow key={p.id} className={cn(!p.enabled && "opacity-70")}>
                    ...
                    <Td className={cn("tabular text-end", p.errorRate24h >= 5 ? "text-danger" : p.errorRate24h >= 1 ? "text-accent-dark" : "text-ink")}>
