                ...
                <div className="space-y-3">
                  <dl className="grid grid-cols-3 gap-3 text-sm">
                    ...
                    <div>
                      ...
                      <dd className={cn("tabular font-semibold", (p.latencyMs ?? 0) > 1000 ? "text-accent-dark" : "text-ink")}>
...
function ServiceCard({ service: s }: { service: ServiceTile }) {
  ...
  return (
    <article
      className={cn(
        "rounded-[var(--radius-card)] border bg-paper-raised p-4",
        s.status === "down" ? "border-danger/40" : s.status === "degraded" ? "border-accent/40" : "border-line"
      )}
    >
