...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  ...
  const description =
    ...
    (product.description ? product.description.replace(/\s+/g, " ").slice(0, 160) : getDictionary(locale).meta.storeDescription(store.name));
          ...
          <div className="space-y-10">
            {product.description && (
              <section aria-labelledby="desc-title">
                <h2 id="desc-title" className="text-xl font-semibold text-ink">
