 ...
 * when the step mounts. The page keys this on the step, so each advance
 * (which refreshes the route in place) plays it again for the new step and
 * never for a re-render of the same one.
 *
 * Web Animations rather than a stylesheet keyframe so the first server paint
 * is the finished state — a shopper with JavaScript still loading sees the
 * page, not a blank waiting for an animation — and so reduced motion is a
 * plain "do nothing".
 */
