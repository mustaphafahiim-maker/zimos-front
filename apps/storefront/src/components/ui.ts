/**
 * Shared class recipes for the storefront's default (ZIMOS) visual language:
 * white/Cloud surfaces, rounded-2xl cards, light-blue hairlines, generous
 * spacing. Every colour is a semantic token so merchant branding (.brand-theme)
 * flows through. Tap targets are ≥ 44px (min-h-11).
 */
...
export const input =
  "block w-full min-h-11 rounded-xl border border-line-strong bg-paper-raised px-3.5 py-2.5 text-base text-ink placeholder:text-ink-muted outline-none transition focus:border-primary focus:ring-4 focus:ring-primary/15 aria-[invalid=true]:border-danger aria-[invalid=true]:ring-danger/10";
